...
public static partial class OxideGizmos
{
    ...
    public static void Sphere([NotNull] Connection connection, Vector3 pos, float radius, Color color, float duration,
        float visibleDistance = float.PositiveInfinity)
    {
        if (connection == null)
            throw new ArgumentNullException(nameof(connection));

        SendHandler.Enqueue(connection, COMMAND_SPHERE, duration, color, pos, radius, visibleDistance);
    }
        ...
        float visibleDistance = float.PositiveInfinity)
    {
        ...
        Sphere(player.Connection, pos, radius, color, duration, visibleDistance);
    }
        ...
        float visibleDistance = float.PositiveInfinity)
    {
        ...
        SendHandler.Enqueue(players, COMMAND_SPHERE, duration, color, pos, radius, visibleDistance);
    }
        ...
        float visibleDistance = float.PositiveInfinity)
    {
        ...
        SendHandler.Enqueue(connections, COMMAND_SPHERE, duration, color, pos, radius, visibleDistance);
    }

    public static void Sphere([NotNull] Connection connection, Vector3 pos, float radius, Vector3 color, float duration,
        float visibleDistance = float.PositiveInfinity)
    {
        if (connection == null)
            throw new ArgumentNullException(nameof(connection));
    ...
    public static void Sphere([NotNull] BasePlayer player, Vector3 pos, float radius, Vector3 color, float duration,
        float visibleDistance = float.PositiveInfinity)
    {
        if (player == null)
            throw new ArgumentNullException(nameof(player));
